...
[Natashekara와 Sampath (2026)](https://doi.org/10.1016/j.jbef.2026.101167)의 CoinMarketCap 62종목 분석 결과를 재현하는 데서 시작해, 다른 기간과 거래소에서도 같은 현상이 나타나는지 확인하고, 마지막에는 **집단추종이 없는 가상시장에서도 같은 신호가 생기는지** 실험했습니다.

## 요약

- 선행논문이 집단추종의 근거로 제시한 통계값을 거의 같은 수치로 재현했습니다.
- 하지만 이 연구에서 만든 집단추종 없는 가상시장에서도 같은 종류의 통계값이 97~100%의 빈도로 나타났습니다.
- 따라서 **논문의 계산 결과는 재현되지만, 그 숫자만으로 투자자가 서로 따라 거래했다고 입증할 수는 없습니다.**
- 1분봉과 개별 체결자료에서도 단기 수익률을 안정적으로 예측하는 신호는 찾지 못했습니다.
- 자동매매에서 반복적으로 수익을 낼 수 있다고 확인된 예측 신호는 없습니다.

> 이 연구의 가장 중요한 성과는 수익 전략을 발견한 것이 아니라, 기존 지표가 잘못 해석될 수 있는 수학적 원인을 확인한 것입니다.

공개된 경량 결과와 검증 스크립트로 주요 판정을 다시 계산할 수 있습니다. 원자료부터 전체 연구를 재실행하려면 별도 시장 데이터를 준비해야 합니다.
